...
const formatCSSDefinition = (name: string, nameImport: string, classNames: string[]) => `\
import Namespace from './${nameImport}ThemeProps.ts'
...
const formatInterfaceCSSDefinition = (classNames: string[]) => `\
...
async function writeCSSDefinition(cssPath: string, classNames: string[]): Promise<string> {
	...
	const nameClear = fixname(path.basename(cssPath, '.css')).split(".")[0] 
	const definition = formatCSSDefinition(name, nameClear, classNames)
	const definitionInterface = formatInterfaceCSSDefinition(classNames)
	const dPath = `${cssPath}.d.ts`
	const diPath = `${cssPath.replace(name, '')}${nameClear}ThemeProps.ts`
	await fs.writeFile(dPath, `${definition}\n`)
	await fs.writeFile(diPath, `${definitionInterface}\n`)
	return dPath
}
